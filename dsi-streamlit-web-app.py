...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 19 14:23:16 2026
...
if st.button("Submit For Prediction"):
    ...
    # store data in a dataframe from prediction
    new_data = pd.DataFrame({"age" : [age], "gender" : [gender], "credit_score" : [credit_score]})
    
    # apply model pipeline to the input data and extract just the numeric probability prediction
    pred_proba = model.predict_proba(new_data)[0][1]
    
    # output prediction
    st.subheader(f"Based on these customer attributes, our model predicts a "
                 f"purchase probability of {pred_proba:.0%}") # :. prints whole number instead
